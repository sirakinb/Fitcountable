...
async function rpc(token: string, name: string, body: Record<string, unknown> = {}) {
  const response = await fetch(`${baseUrl}/api/database/rpc/${name}`, {
    method: "POST",
...
async function socialHandler(
  req: Request,
  run: (token: string, body: Record<string, unknown>, userId: string) => Promise<unknown>
) {
  if (req.method === "OPTIONS") return new Response(null, { status: 204, headers: corsHeaders });
  if (req.method !== "GET" && req.method !== "POST") return json({ error: "Method not allowed" }, 405);
  const token = req.headers.get("Authorization")?.replace(/^Bearer\s+/i, "") ?? "";
  const userId = decodeJwtSubject(token);
  if (!token || !userId) return json({ error: "Authorization bearer token is required" }, 401);
  const url = new URL(req.url);
  const queryBody = Object.fromEntries(url.searchParams.entries());
  const requestBody = req.method === "POST" ? await req.json().catch(() => ({})) : {};
  try {
    return json(await run(token, { ...queryBody, ...requestBody }, userId));
  } catch (error) {
...
export default async function(req: Request): Promise<Response> {
  return socialHandler(req, async (token) => ({
    result: await rpc(token, "fc_delete_account_data")
  }));
}
