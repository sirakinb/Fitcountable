...
export default async function(req: Request): Promise<Response> {
  return socialHandler(req, async (token, body) => ({
    profile: await rpc(token, "fc_bootstrap_profile", {
      p_display_name: body.display_name ?? body.displayName ?? null,
      p_avatar_url: body.avatar_url ?? body.avatarURL ?? null,
      p_goal_type: body.goal_type ?? body.goalType ?? null,
      p_privacy_mode: body.privacy_mode ?? body.privacyMode ?? null
    })
  }));
}
